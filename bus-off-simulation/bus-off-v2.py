...
# Standard-library modules used for timing, concurrency, queues, statistics, and data containers.
import time
import threading
import queue
import random
...
# python-can is optional; the pure-Python simulator remains usable when the package is unavailable.
try:
...
# Linux SocketCAN flags used to construct diagnostic protocol-error frames.
CAN_ERR_FLAG = 0x20000000
CAN_ERR_PROT = 0x00000008  # Protocol-violation error class used here for bit-monitoring diagnostics.
...
# Send a normal diagnostic data frame identifying an ECU active-error indication.
...
# Serialize up to eight bytes into the native SocketCAN can_frame layout and transmit it.
def sockcan_send_data(sock, arb_id: int, data: bytes):
    dlc = min(len(data), 8)
    frame = struct.pack("=IB3x8s", arb_id & 0x1FFFFFFF, dlc, data[:8].ljust(8, b"\x00"))
    sock.send(frame)

# Send a SocketCAN protocol-error frame containing compact diagnostic metadata.
def sockcan_send_error_active(sock, ecu_name: str, offender_code: int = 1):
    ...
    # The free-form payload is displayed by candump: ECU initial, "E", "A", and offender code.
    payload = bytes([ord(ecu_name[0]) & 0xFF, ord('E'), ord('A'), offender_code & 0xFF]).ljust(8, b"\x00")
    frame = struct.pack("=IB3x8s", can_id, 8, payload)
    sock.send(frame)


# Represent the two physical CAN bus levels; dominant zero overrides recessive one.
class BitValue(Enum):
    DOMINANT = 0
    RECESSIVE = 1
...
# Enumerate the master state-machine phases that coordinate bus activity.
...
# Label every emitted bit with its logical CAN frame field for diagnostics and arbitration.
class Field(Enum):
    SOF = auto()
    ID = auto()
    RTR = auto()
...
# Store one ECU transmission request submitted to the master arbitration queue.
@dataclass
...
# Describe one resolved bus bit broadcast to every simulated ECU receive queue.
@dataclass
...
# Construct a complete base-frame bit sequence and retain one field label per bit.
class CANBitStream:
    """Build a *stuffed* CAN base frame bit stream and per-bit field labels."""

    # Normalize the arbitration ID and payload before building the frame once.
    ...
    # Append one bit and its matching field label while keeping both arrays synchronized.
    def _push(self, bit: BitValue, field: Field):
        self.bits.append(bit)
        self.fields.append(field)

    # Assemble the logical CAN frame before inserting bit-stuffing bits.
    def _build_unstuffed_payload_bits(self) -> Tuple[List[BitValue], List[Field]]:
        bits: List[BitValue] = []
        fields: List[Field] = []

        # Local helper for appending matching bit and field entries.
    ...
    # Insert a complementary bit after five identical bits in the stuffable frame region.
    @staticmethod
    def _apply_bit_stuffing(bits: List[BitValue], fields: List[Field]) -> Tuple[List[BitValue], List[Field]]:
        ...
        # Bit stuffing applies from SOF through CRC, but not to delimiters or EOF.
        ...
        # Scan the raw stream once while tracking consecutive equal bus values.
        for i, (b, f) in enumerate(zip(bits, fields)):
            ...
            # Break each five-bit run by appending the opposite bus value as a STUFF field.
            if run_len == 5:
    ...
    # Build the raw frame first, then expose the stuffed bit and field sequences.
    def _build(self):
...
class FaultInjector:
    ...
    # Zero disables R0 injection; a positive value injects once every N frames.
    def __init__(self, *, flip_r0_every_n_frames: int = 0):
        self.flip_r0_every_n_frames = flip_r0_every_n_frames
        self._frame_count = 0

    # Advance the injector frame counter at the beginning of each arbitration cycle.
    def on_new_frame(self):
        self._frame_count += 1

    # Optionally force R0 dominant on selected frames while leaving all other bits unchanged.
...
# Model one CAN ECU, including scheduling, fault counters, and per-bit transmission state.
class BaseECU:
    # Initialize identity, scheduling, worker threads, frame progress, and error-flag state.
    def __init__(
        self,
        name: str,
        slave_id: int,
        ...
        # Each ECU owns an independent periodic schedule and configurable initial delay.
        self.tx_period = tx_period
        self.start_delay = start_delay
        self.next_tx_time = 0.0

        # TEC and REC determine the current CAN fault-confinement state.
        self.tec = 0
        self.rec = 0
        self.state = NodeState.ERROR_ACTIVE

        # Separate daemon threads create outgoing requests and consume bus broadcasts.
        self._stop = False
    ...
    # Start receive processing immediately and periodic transmission only when enabled.
    def start(self):
    ...
    # Queue one deterministic frame without relying on the periodic scheduling thread.
    def request_once(self, data: bytes, *, arbitration_id: Optional[int] = None):
        ...
        if self._pending_req is not None:
            ...
            return
        # Capture one timestamp and clamp an optional arbitration ID to eleven bits.
        now = time.time()
    ...
    # Signal both ECU worker loops to stop at their next condition check.
    def stop(self):
        self._stop = True

    # Recompute ERROR_ACTIVE, ERROR_PASSIVE, or BUS_OFF from the current TEC threshold.
    ...
    # BUS_OFF nodes are excluded from new requests, arbitration, and retransmission.
    def is_bus_off(self) -> bool:
        return self.state is NodeState.BUS_OFF

    # A frame is eligible only when both request metadata and its bitstream are available.
    ...
    # Lazily construct the bitstream when the master is ready to arbitrate the request.
    ...
    # Switch from frame transmission to a finite active or passive error-flag emission.
    ...
    # Let the master identify an ECU currently forcing dominant active-error bits.
    def is_emitting_error_flag_active(self) -> bool:
        return self._tx_mode == "ERR_ACTIVE"

    # Return the next bus contribution without advancing frame or error-flag progress.
    def peek_next_bit(self) -> Optional[Tuple[BitValue, Field]]:
        ...
        # Normal transmission reads from the prebuilt stuffed frame at the current cursor.
    ...
    # Consume one emitted error bit or advance one position in the normal frame.
    def advance(self):
    ...
    # Detect a bit-monitoring mismatch outside the arbitration window.
    def on_bus_bit(self, bus_bit: BitValue, field: Field, *, arbitration_window: bool):
        ...
        # Recessive-versus-dominant mismatch is expected in arbitration but is an error afterward.
    ...
    # Apply bounded counter deltas and immediately refresh the ECU state.
    ...
    # Successful transmission gradually recovers the transmitter by reducing TEC.
    def apply_success_update(self):
        """Apply the successful-transmission rule by decrementing TEC by one."""
        self.tec = max(0, self.tec - 1)
        self._update_state()

    # Clamp overdue schedules to zero for a clean human-readable status display.
    ...
    # Periodically submit payloads without blocking the master state-machine thread.
    def _tx_loop(self):
        if not self.auto_tx:
            return
        
        # BUS_OFF ECUs remain alive as threads but cannot generate additional traffic.
    ...
    # Consume broadcast bits; synchronous transmitter monitoring is performed by the master.
    def _rx_loop(self):
        while not self._stop:
...
# Coordinate arbitration, bus resolution, error handling, broadcasts, and frame lifecycle.
class CANMaster:
    # Initialize timing, queues, state data, fault injection, and optional SocketCAN output.
    def __init__(
        self,
        tick_ms: float = 0.1,
        ...
        # Each registered ECU has a receive queue; transmission intents share one master queue.
        ...
        # One worker advances the bus while another periodically reports status.
        self._stop = False
        ...
        # Per-frame error state records senders, delimiters, and whether the frame remains valid.
        # Error handling
        self._error_senders: List[int] = []  # Every ECU required to emit an error flag for this frame.
        ...
        # Active error flags are handled inside TRANSMIT so their dominant bits remain visible.
        ...
        # SocketCAN forwarding is best-effort and does not prevent pure simulation.
        # SocketCAN
    ...
    # Queue a transmission intent for collection by the master loop.
    def submit_request(self, req: TransmissionRequest):
        self._pending_starts.put(req)

    # Wait briefly for the next broadcast bit delivered to one ECU receive queue.
    ...
    # Launch the master bus loop and the independent status reporter.
    def start(self):
        print("[MASTER] START")
        self._thread.start()
        self._status_thread.start()

    # Request shutdown and wait briefly for the main master worker to terminate.
    def stop(self):
        self._stop = True
        self._thread.join(timeout=2)
        print("[MASTER] STOP")

    # Print state only while IDLE to avoid interleaving status blocks with bit traces.
    ...
    # Process one transmitter bit-monitoring error according to its pre-error state.
    def report_bit_error(self, sender_slave_id: int, offender: str = "unknown"):
        ...
        # Select ACTIVE or PASSIVE behavior from the state before applying the TEC penalty.
        pre_state = sender.state

        # -------------------------------
        # ERROR PASSIVE: send recessive error flag, do NOT abort the frame
        # -------------------------------
        # A passive flag is recessive and therefore cannot overwrite another dominant sender.
        ...
        # -------------------------------
        # An active flag drives dominant and invalidates the frame for every participant.
        ...
        # Penalize the detecting transmitter and redirect its output to the active error flag.
        sender.apply_error_update(tec_delta=8)
        sender._enter_error_flag(active=True)
        print(f"[MASTER] {sender.name}: TEC={sender.tec} (+8 for ACTIVE error) State={sender.state.name}")

        # Listening nodes increment REC once; active transmitters are excluded.
    ...
    # Drain all queued requests and prepare the corresponding ECU bitstreams.
    def _drain_start_intents(self) -> List[int]:
        """Collect ALL pending start intents."""
        new_contenders: List[int] = []
        while True:
            # Non-blocking access stops immediately when no additional intents are available.
            try:
    ...
    # Collect pending ECUs, wait for the deadline, and initialize a new arbitration round.
    def _handle_idle(self):
        ...
        # Use one timestamp for every gather-window decision in this iteration.
        now = time.time()
    ...
    # Read one contender arbitration bit without changing its cursor.
    ...
    # Resolve wired-AND arbitration one bit at a time and remove losing nodes.
    def _handle_arbitration_step(self):
        self._arbitration_window = True

        # Gather one offered bit from every contender still eligible in this tick.
        ...
        # Print each arbitration bit together with every contender contribution.
        ...
        # An ECU loses by sending recessive while observing dominant in ID or RTR.
        ...
        # A single remaining contender becomes the winner and begins normal transmission.
        ...
        # Multiple nodes surviving through RTR have identical arbitration fields.
    ...
    # Resolve sender bits, perform bit monitoring, and advance or terminate the frame.
    def _handle_transmit_step(self):
        assert self._winner is not None

        # Requests arriving while the bus is busy remain pending for a later arbitration.
        ...
        # No offered bits means all active senders completed or abandoned their contribution.
        if not offered:
            for sid in self._active_senders:
        ...
        # Divergent field labels reveal desynchronized streams, so label the bit as STUFF.
        field = offered[0][2]
        ...
        # Resolve the wired-AND bus value before applying any deterministic fault override.
        ...
        # Print each transmitted bit and, during collisions, every active sender contribution.
        ...
        # Let every active transmitter compare its intended bit with the resolved bus.
        for sid, _b, _f in offered:
    ...
    # Emit the legacy global error-flag sequence for the recorded error senders.
    def _handle_error_flag_step(self):
        ...
        # Classify error-flag senders according to their current fault-confinement state.
        active_senders = [
        ...
        # Consume one emitted error-flag bit from the remaining-bit counter.
    ...
    # Emit the recessive delimiter separating the error flag from the interframe period.
    ...
    # Finalize successful frames, update counters, or preserve failed requests for retransmission.
    def _handle_eof(self):
        if self._frame_ok and self._winner is not None:
            ...
            # Record only ECUs that reached the end of an intact normal frame.
            ...
            # Forward and account for the frame only when at least one sender completed it.
            if successful_senders:
                ...
                # Apply successful-transmission recovery and clear completed requests.
                for sid in successful_senders:
                    ecu = self.slaves[sid]
                    ecu.apply_success_update()
                    print(f"[MASTER] {ecu.name}: TEC={ecu.tec} (-1 after successful transmission) State={ecu.state.name}")
                ...
                for sid, ecu in self.slaves.items():
                    ...
                    if ecu.rec > 0:
                        ecu.rec -= 1
                        print(f"[MASTER] {ecu.name}: REC={ecu.rec} (-1 after successfully receiving a valid frame)")
        else:
            # Failed frames are announced before preserved requests are rewound to SOF.
    ...
    # Clear all per-frame state and return the master to IDLE.
    def _handle_interframe(self):
        ...
        # Restore every non-BUS_OFF ECU to normal bit-output mode for the next frame.
...
# Configure the demonstration topology and run until BUS_OFF or manual interruption.
def main():
    print("=" * 100)
    ...
    print("  - Collision: BOTH ECUs increment TEC by +8")
    ...
    # Create the master with a fine-grained simulation tick and no mandatory SocketCAN forwarding.
    master = CANMaster(
        tick_ms=0.5,
    ...
    # ========================================
    # The start times below deliberately create overlapping transmission requests:
    ...
    # Display the configured identifiers and periods before starting ECU workers.
    ...
    # Start ECUs only after the master is ready to consume their requests.
    ecu_a.start()
    ecu_b.start()
    ecu_c.start()
    ecu_d.start()
    ...
    try:
        while True:
            ...
            # Stop the demonstration when any ECU reaches the BUS_OFF state.
            # Check BUS-OFF
    ...
    # Stop all workers after printing the final fault-confinement counters.
    # Cleanup
    print(f"\n{'='*100}")
    print("[FINAL STATUS]")
    print(f"{'='*100}")
